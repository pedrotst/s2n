...
static int s2n_sslv3_client_finished(struct s2n_connection *conn)
{
    ...
    lte_check(MD5_DIGEST_LENGTH + SHA_DIGEST_LENGTH, sizeof(conn->handshake.client_finished));
    GUARD(s2n_hash_copy(&conn->handshake.md5_copy_working_space, &conn->handshake.md5));
    GUARD(s2n_hash_copy(&conn->handshake.sha1_copy_working_space, &conn->handshake.sha1));
    return s2n_sslv3_finished(conn, prefix, &conn->handshake.md5_copy_working_space, &conn->handshake.sha1_copy_working_space, conn->handshake.client_finished);
}
...
static int s2n_sslv3_server_finished(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_hash_copy(&conn->handshake.md5_copy_working_space, &conn->handshake.md5));
    GUARD(s2n_hash_copy(&conn->handshake.sha1_copy_working_space, &conn->handshake.sha1));
    return s2n_sslv3_finished(conn, prefix, &conn->handshake.md5_copy_working_space, &conn->handshake.sha1_copy_working_space, conn->handshake.server_finished);
}
...
int s2n_prf_client_finished(struct s2n_connection *conn)
{
    ...
    if (conn->actual_protocol_version == S2N_TLS12) {
        ...
        case S2N_HMAC_SHA256:
            GUARD(s2n_hash_copy(&conn->handshake.hash_copy_working_space, &conn->handshake.sha256));
            GUARD(s2n_hash_digest(&conn->handshake.hash_copy_working_space, sha_digest, SHA256_DIGEST_LENGTH));
            sha.size = SHA256_DIGEST_LENGTH;
            break;
        case S2N_HMAC_SHA384:
            GUARD(s2n_hash_copy(&conn->handshake.hash_copy_working_space, &conn->handshake.sha384));
            GUARD(s2n_hash_digest(&conn->handshake.hash_copy_working_space, sha_digest, SHA384_DIGEST_LENGTH));
    ...
    GUARD(s2n_hash_digest(&conn->handshake.md5_copy_working_space, md5_digest, MD5_DIGEST_LENGTH));
    GUARD(s2n_hash_digest(&conn->handshake.sha1_copy_working_space, sha_digest, SHA_DIGEST_LENGTH));
    md5.data = md5_digest;
