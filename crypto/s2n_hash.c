...
static int s2n_low_level_hash_wipe(struct s2n_hash_state *state)
{
    return 0;
}
...
static int s2n_evp_hash_wipe(struct s2n_hash_state *state) 
{
    int r;
...
int s2n_hash_wipe(struct s2n_hash_state *state)
{
    return state->hash_impl->wipe(state);
}
